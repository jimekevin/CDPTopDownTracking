...
#define RECORDING_TABLE_3 "/Users/lilith/Projekte/CDPTopDownTracking/TopDownTracking/data/20210617_165711.bag"
...
enum RECORDING { NONE=-1, LIVE, WT_BS, WT_WS, BT_BS, BT_WS, TABLE_0, TABLE_1, TABLE_2, TABLE_3, COUNT };
inline const std::string getRecording(int id) {
  ...
  switch (rec) {
    ...
    case TABLE_3: return RECORDING_TABLE_3;
    default:    return "";
  }
}

int activeRecording = static_cast<int>(RECORDING::TABLE_3);
...
int main(int, char**) try
{
  ...
  while (app) // Application still alive?
  {
    ...
    if (!rcm->ProcessFrames()) {
      std::cout << "Process frames failed " << frame_count << "\n";
      failed_count += 1;
      if (failed_count >= failed_max) {
        std::cout << "Failed too many times (" << failed_max << "), ABORT\n";
        break;
      }
      continue;
    } else {
